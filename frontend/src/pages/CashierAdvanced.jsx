...
const CashierAdvanced = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [statsData, invoicesData, transactionsData, servicesData, categoriesData, patientsData] = await Promise.all([
        billingService.getStats(),
        billingService.getInvoices({ limit: 20 }),
        billingService.getTransactions({ limit: 10 }),
        servicesService.getServices(),
        billingService.getServiceCategories(),
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      if (response.success) {
        showAlert('Hisob-faktura muvaffaqiyatli yaratildi!', 'success', 'Muvaffaqiyatli');
        // Reset form
  ...
  const handleServiceSubmit = async (e) => {
    ...
    try {
      const data = {
        ...
        category: serviceForm.category_id || 'Umumiy',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="material-symbols-outlined text-3xl opacity-80">receipt_long</span>
            <span className="text-sm opacity-80">Kutilmoqda</span>
          </div>
          <p className="text-3xl font-black">{stats.pendingInvoices || 0}</p>
          <p className="text-sm opacity-80 mt-1">To'lanmagan fakturalar</p>
        </div>

        <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="material-symbols-outlined text-3xl opacity-80">account_balance_wallet</span>
            <span className="text-sm opacity-80">Qarz</span>
          </div>
          <p className="text-3xl font-black">{formatCurrency(stats.totalDebt || 0)}</p>
          <p className="text-sm opacity-80 mt-1">Jami qarz</p>
        </div>

        <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="material-symbols-outlined text-3xl opacity-80">payments</span>
            <span className="text-sm opacity-80">Naqd</span>
          </div>
          <p className="text-3xl font-black">{formatCurrency(stats.todayByMethod?.cash || 0)}</p>
          <p className="text-sm opacity-80 mt-1">Naqd to'lovlar</p>
        </div>

        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="material-symbols-outlined text-3xl opacity-80">credit_card</span>
            <span className="text-sm opacity-80">Karta</span>
          </div>
          <p className="text-3xl font-black">{formatCurrency(stats.todayByMethod?.card || 0)}</p>
          <p className="text-sm opacity-80 mt-1">Kartadan to'lovlar</p>
        </div>

        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <span className="material-symbols-outlined text-3xl opacity-80">account_balance</span>
            <span className="text-sm opacity-80">O'tkazma</span>
          </div>
          <p className="text-3xl font-black">{formatCurrency(stats.todayByMethod?.transfer || 0)}</p>
          <p className="text-sm opacity-80 mt-1">O'tkazmalar to'lovlari</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div>
                  ...
                  {selectedPatient && (
                    <div className="mt-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg flex items-center justify-between">
                      <div>
            ...
            <div className="space-y-4">
              ...
              {services.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">Xizmatlar yo'q</p>
                </div>
              ) : (
