  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-white">Outpatient Chairs</h1>
        ...
        <div className="flex gap-3">
          <div className="px-4 py-2 bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-700 rounded-lg">
            ...
            <p className="text-2xl font-black text-green-600">{getNextAvailableTime()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Chairs</p>
              <p className="text-3xl font-black text-gray-900 dark:text-white">{chairs.length}</p>
            </div>
            <div className="size-12 bg-primary/10 text-primary rounded-lg flex items-center justify-center">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-black text-green-600">{occupiedCount}</p>
            </div>
            <div className="size-12 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-lg flex items-center justify-center">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Avg Treatment</p>
              <p className="text-3xl font-black text-primary">32min</p>
            </div>
            <div className="size-12 bg-primary/10 text-primary rounded-lg flex items-center justify-center">
      ...
      {/* Visual Seat Map */}
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <h2 className="text-xl font-bold mb-6">Chair Map</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {chairs.map((chair) => (
            <button
              key={chair.id}
              onClick={() => setSelectedChair(chair)}
              className={`p-6 rounded-xl border-2 transition-all hover:shadow-lg ${getChairColor(chair.status, chair.payment)}`}
            >
              ...
              {chair.status === 'occupied' && (
                <>
                  <p className="text-sm font-bold mb-1 truncate">{chair.patient}</p>
      ...
      {/* Detailed List */}
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <h2 className="text-xl font-bold mb-6">Active Treatments</h2>
        <div className="space-y-3">
          {chairs.filter(c => c.status === 'occupied').map((chair) => (
            <div key={chair.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div className="flex items-center gap-4">
                <div className="size-12 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-lg flex items-center justify-center font-bold">
                  {chair.number}
                </div>
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white">{chair.patient}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{chair.treatment} • Started {chair.startTime}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <p className="text-lg font-bold text-primary">{chair.duration - chair.elapsed} min</p>
                </div>
                <button
      ...
      {showNotification && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-md w-full animate-bounce">
            <div className="p-6 bg-green-600 rounded-t-2xl text-white text-center">
              <span className="material-symbols-outlined text-6xl mb-2">check_circle</span>
              <h2 className="text-2xl font-black">Patient Ready!</h2>
            </div>
            <div className="p-6 text-center">
              <p className="text-lg font-semibold mb-2">Chair C-01</p>
              <p className="text-gray-600 dark:text-gray-400 mb-4">Aziza Karimova</p>
              <p className="text-sm text-gray-500 mb-6">Treatment completed. Patient ready for doctor consultation.</p>
              <button
                onClick={() => setShowNotification(false)}
                className="w-full px-6 py-3 bg-primary text-white rounded-lg font-bold hover:opacity-90"
              >
                Acknowledge
              </button>
            </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-md w-full">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <h2 className="text-2xl font-black">Chair {chair.number}</h2>
        ...
        <div className="p-6 space-y-4">
          {chair.status === 'occupied' && (
            <>
              <div>
                ...
                <p className="text-lg font-bold">{chair.patient}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Treatment</p>
                <p className="text-lg font-bold">{chair.treatment}</p>
              </div>
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold">{Math.round((chair.elapsed / chair.duration) * 100)}%</span>
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                  {chair.elapsed} of {chair.duration} minutes
                </p>
              </div>
              <button className="w-full px-4 py-3 bg-red-600 text-white rounded-lg font-semibold hover:opacity-90">
                End Treatment
              </button>
            </>
          )}
          ...
          {chair.status === 'available' && (
            <div className="text-center py-8">
              ...
              <p className="text-lg font-bold text-green-600">Chair Available</p>
              <button className="mt-6 w-full px-4 py-3 bg-primary text-white rounded-lg font-semibold hover:opacity-90">
                Assign Patient
              </button>
            </div>
          )}
