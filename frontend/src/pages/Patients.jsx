  ...
  return (
    <div className="p-3 sm:p-4 sm:p-4 sm:p-6 lg:p-4 sm:p-6 lg:p-8 space-y-3 sm:space-y-4 sm:space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
        <div>
          <h1 className="text-2xl sm:text-2xl sm:text-3xl font-black text-gray-900 dark:text-white">{t('patients.title')}</h1>
      ...
      {/* Search Bar */}
      <div className="bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl border border-gray-200 dark:border-gray-800 p-3 sm:p-3 sm:p-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 sm:pl-12 pr-4 py-2 sm:py-2.5.5 sm:py-2 sm:py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg sm:rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm sm:text-sm sm:text-base"
          />
          {loading && (
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden sm:block">
        {loading && patients.length === 0 ? (
          ...
          <>
            ...
            <div className="hidden lg:block overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                  <tr>
                    <th className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-2 sm:py-3 text-left text-xs font-bold text-gray-600 dark:text-gray-400 uppercase">
                      ID
                    </th>
                    <th className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-2 sm:py-3 text-left text-xs font-bold text-gray-600 dark:text-gray-400 uppercase">
                      {t('patients.fullName')}
                    ...
                    <th className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-2 sm:py-3 text-right text-xs font-bold text-gray-600 dark:text-gray-400 uppercase">
                      {t('patients.actions')}
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {patients.map((patient) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-3 sm:py-4 text-sm sm:text-sm sm:text-base font-semibold text-primary">
                        ...
                        <td className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-3 sm:py-4 text-sm sm:text-sm sm:text-base text-gray-600 dark:text-gray-400">
                          {formatPhone(patient.phone)}
                        </td>
                        <td className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
                          {patient.access_code ? (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300 rounded-lg sm:rounded-lg sm:rounded-xl font-bold text-sm sm:text-sm sm:text-base tracking-wider">
                        ...
                        <td className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
                          <span className={`text-sm sm:text-sm sm:text-base font-bold ${
                        ...
                        <td className="px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-3 sm:py-4 text-right">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
            ...
            <div className="lg:hidden sm:block divide-y divide-gray-200 dark:divide-gray-700">
              {patients.map((patient) => {
                const statusConfig = getStatusConfig(
                  patient.current_balance || 0, 
                  patient.debt_limit || 500000
                  ...
                  >
                    ...
                    <div className="flex items-start gap-2 sm:gap-3 mb-3">
                      {patient.photo_url ? (
                        ...
                        <div className="size-12 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-base sm:text-lg flex-shrink-0">
                          {patient.first_name?.[0] || '?'}{patient.last_name?.[0] || ''}
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 sm:gap-2 sm:gap-3">
                    ...
                    {/* Info Grid */}
                    <div className="space-y-2 sm:space-y-2 sm:space-y-3 text-sm sm:text-sm sm:text-base">
                      {/* Access Code - Telegram bot uchun */}
                      {patient.access_code && (
                        <div className="flex items-center justify-between gap-2 sm:gap-2 sm:gap-3 bg-green-50 dark:bg-green-900/20 p-2 rounded-lg sm:rounded-lg sm:rounded-xl border border-green-200 dark:border-green-800">
                          <span className="text-green-700 dark:text-green-400 flex items-center gap-1.5 text-xs">
                            <span className="material-symbols-outlined text-sm sm:text-base">qr_code</span>
                            Telegram kod
                          </span>
                          <span className="font-bold text-green-900 dark:text-green-300 text-sm sm:text-base tracking-wider">
                            {patient.access_code}
                          </span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between gap-2 sm:gap-2 sm:gap-3">
                        <span className="text-gray-600 dark:text-gray-400 flex items-center gap-1.5">
                          <span className="material-symbols-outlined text-sm sm:text-base">cake</span>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                      <button
                        ...
                        className="w-full px-4 sm:px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-2 sm:py-2.5 bg-primary/10 text-primary rounded-lg sm:rounded-lg sm:rounded-xl text-sm sm:text-sm sm:text-base font-semibold hover:bg-primary/20 transition-colors flex items-center justify-center gap-2 sm:gap-2 sm:gap-3"
                      >
            ...
            {pagination.total > pagination.limit && (
              <div className="px-4 sm:px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 sm:px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-3 sm:py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-col sm:flex-row items-center justify-between gap-2 sm:gap-3">
                <p className="text-xs sm:text-sm sm:text-sm sm:text-base text-gray-600 dark:text-gray-400">
                  {((pagination.page - 1) * pagination.limit) + 1} - {Math.min(pagination.page * pagination.limit, pagination.total)} / {pagination.total}
                </p>
                <div className="flex gap-2 sm:gap-2 sm:gap-3 w-full sm:w-auto">
                  <button
                    onClick={() => setPagination(prev => ({ ...prev, page: prev.page - 1 }))}
                    disabled={pagination.page === 1}
                    className="flex-1 sm:flex-none px-4 sm:px-4 sm:px-6 lg:px-4 sm:px-6 lg:px-8 py-2 sm:py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg sm:rounded-lg sm:rounded-xl text-sm sm:text-sm sm:text-base font-semibold hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {t('patients.previous')}
                  </button>
                  <button
