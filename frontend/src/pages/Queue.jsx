  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-white">Queue Management</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Today's appointments and waiting list</p>
        </div>
        <button className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold hover:opacity-90 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            <div className={`size-12 ${stat.color} rounded-lg flex items-center justify-center text-white mb-3`}>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <h2 className="text-xl font-bold mb-6">Current Queue</h2>
            ...
            <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">{item.patientId} • {item.doctor}</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <span className="text-sm font-semibold">{item.time}</span>
                <StatusBadge status={item.status === 'in-progress' ? 'warning' : 'info'} text={item.status} />
                <button className="px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold">Call Next</button>
              </div>
            </div>
          ))}
        </div>
